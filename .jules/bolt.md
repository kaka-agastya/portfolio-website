...
## 2024-07-27 - Replacing Heavy Animation Libraries with Native Browser APIs
**Learning:** Using heavy animation libraries like Framer Motion for simple enter/exit transitions or visibility reveals significantly inflates the client-side JavaScript bundle size. For basic animations, the cost of shipping the library outweighs its convenience.
**Action:** Replace simple animations (like revealing elements on scroll or fading out loading screens) with native `IntersectionObserver` and CSS transitions. This drastically reduces the client JS bundle without sacrificing visual fidelity.
