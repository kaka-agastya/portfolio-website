...
## 2026-08-02 - Singleton IntersectionObserver
**Learning:** Creating new IntersectionObservers for every item in a list creates unnecessary memory overhead (O(n)).
**Action:** Use a module-level Singleton IntersectionObserver with a WeakMap to map DOM elements to their visibility state callbacks (O(1)).
