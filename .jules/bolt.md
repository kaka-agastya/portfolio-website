...
## 2025-07-26 - Replace framer-motion with Native Browser APIs for Simple Animations
**Learning:** For portfolios and primarily static sites, using a heavy animation library like `framer-motion` for basic scroll-reveal or loading fade-out effects significantly bloats the JS bundle size and delays time-to-interactive. Simple transitions (fade in, translate up) can be perfectly replicated using native `IntersectionObserver` and CSS transitions without any visual degradation.
**Action:** Default to `IntersectionObserver` and standard CSS transitions/animations for simple entrance effects. Reserve heavy animation libraries strictly for complex, orchestrated, or multi-step interactive sequences where native CSS becomes unwieldy.
