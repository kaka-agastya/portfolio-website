...
## 2024-05-18 - Skip to Main Content Link Placement
**Learning:** When implementing skip links, <nav> elements must be placed outside the target <main> element so the skipped region does not include navigation. The target element requires tabIndex={-1} for programmatic focus and outline-none to prevent visual artifacts.
**Action:** Ensure structural semantic tags like <nav> are outside <main> and apply proper focus management attributes to target elements.
