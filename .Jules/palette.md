...
## 2024-05-24 - Skip to Main Content
**Learning:** When implementing 'Skip to main content' links, <nav> elements must be placed outside the target <main> element so the skipped region does not include navigation. The target element must have tabIndex={-1} to receive programmatic focus, and outline-none to prevent visual artifacts like page-wide focus rings.
**Action:** Apply this structure for 'Skip to main content' features to ensure correct programmatic focus and styling.
