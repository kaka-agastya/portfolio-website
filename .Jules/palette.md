...
## 2025-02-23 - Skip to Main Content Link
**Learning:** Screen reader and keyboard-only users often have to tab through long navigation menus to reach the primary content of a page. A "Skip to main content" link at the very beginning of the document allows them to bypass these repetitive links. The target container (`<main>`) must have `tabIndex={-1}` and an `outline-none` style (or similar) to successfully receive focus programmatically without showing an awkward focus ring over the entire page.
**Action:** Implement visually hidden (but focusable) "Skip to main content" links in the main layout wrapper and ensure the destination element is appropriately configured to receive focus.
