...
## 2026-07-28 - Skip to Main Content & Programmatic Focus
**Learning:** Screen reader and keyboard users need a way to bypass repetitive navigation blocks (like a sticky Nav) to reach the primary content directly. However, simply linking to a `<main>` tag is insufficient if the element cannot receive focus. If you make it focusable (`tabIndex={-1}`), the browser may draw an unwanted global focus ring around the entire main content area when it receives programmatic focus.
**Action:** Implement a visually hidden "Skip to main content" link as the first focusable element. Ensure the target (e.g. `<main>`) has `id="main-content"`, `tabIndex={-1}` so it can receive focus programmatically, and `className="focus:outline-none"` (or equivalent CSS) to suppress visual artifacts when focus shifts.
