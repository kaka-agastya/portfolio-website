...
## 2025-02-24 - Skip to Main Content Link Accessibility
**Learning:** For a skip link to work perfectly, it requires three things: 1) It must be the first focusable element. 2) The target `<main>` element must have `tabIndex={-1}` and `outline-none` so it can receive programmatic focus without showing an unwanted focus ring. 3) Global navigation like `<nav>` must be structurally placed outside of the `<main>` element, otherwise the user skips right past the navigation completely but still lands inside a container that includes the navigation visually.
**Action:** Always verify that `<nav>` is placed outside of the target `<main>` element, and ensure the target element uses `tabIndex={-1}` when implementing skip links.
