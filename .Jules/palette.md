...
## 2026-07-31 - Skip to Main Content Implementation
**Learning:** Implementing 'Skip to main content' requires placing '<nav>' elements outside the target '<main>' so the skipped region excludes navigation. The target must have 'tabIndex={-1}' for programmatic focus and 'outline-none' to prevent page-wide focus rings.
**Action:** Always verify structural HTML semantics (main, nav) before implementing skip links, and use 'tabIndex={-1}' with 'outline-none' on programmatic focus targets.
