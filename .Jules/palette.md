...
## 2024-03-20 - Skip to Main Content Link
**Learning:** Implementing "Skip to main content" links requires placing `<nav>` elements outside the target `<main>` element so the skipped region does not include navigation. Additionally, the target element must have `tabIndex={-1}` to receive programmatic focus, and `outline-none` (or similar) to prevent visual artifacts like page-wide focus rings.
**Action:** Use this structure for future layout updates to ensure keyboard users can effectively skip navigation and go straight to the main content.
