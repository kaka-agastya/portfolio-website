...
**Learning:** Providing a "Skip to main content" link is critical for keyboard and screen reader users to bypass repetitive navigation elements. The target region must have `tabIndex={-1}` to receive focus programmatically, and `outline-none` to avoid confusing visual artifacts.
**Action:** Always include a visually hidden, focusable skip link at the top of the page hierarchy pointing to the primary `<main>` content area.
