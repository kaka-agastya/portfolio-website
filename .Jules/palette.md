...
## 2025-10-24 - Skip to Main Content Link Pattern
**Learning:** Adding a "Skip to main content" link is critical for keyboard users to bypass repetitive navigation. If the `<nav>` is inside the skipped `<main>` tag, the user will still have to tab through navigation. Also, programmatically focusing the main element after clicking the skip link will show an unwanted focus ring unless `outline-none` is applied.
**Action:** Place `<nav>` outside the `<main>` element, use `href="#main-content"` on the skip link, and ensure the target `<main>` tag has `id="main-content"`, `tabIndex={-1}`, and `outline-none` (or similar utility classes) to handle programmatic focus cleanly.
